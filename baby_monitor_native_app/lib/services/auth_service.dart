...
class AuthService {
  ...
  static Future<void> deleteAccount() async {
    try {
      ...
      // 1. Datos biométricos
      try {
        await supabase.from('face_biometrics').delete().eq('user_id', userId);
        debugPrint('✅ Datos biométricos eliminados');
      } catch (_) {}

      // 2. Datos de bebés (tablas reales del schema)
      try {
        ...
        for (final profile in babyProfiles as List) {
          ...
          for (final table in [
            'feeding_entries', 'sleep_sessions', 'medical_appointments',
            'medicines', 'vaccine_records', 'diary_milestones',
            'camera_configs', 'camera_streams', 'ptz_commands',
          ]) {
            try {
              await supabase.from(table).delete().eq('baby_id', babyId);
            } catch (_) {}
          }
        }
      ...
      // 3. Perfiles de bebés
      try {
        await supabase.from('baby_profiles').delete().eq('user_id', userId);
        debugPrint('✅ Perfiles de bebés eliminados');
      } catch (_) {}

      // 4. Vínculos de familia
      try {
        await supabase.from('family_links').delete().eq('owner_user_id', userId);
        await supabase.from('family_links').delete().eq('caregiver_user_id', userId);
      } catch (_) {}

      // 5. Datos de cámara del usuario
      try {
        await supabase.from('camera_configs').delete().eq('user_id', userId);
        await supabase.from('camera_streams').delete().eq('user_id', userId);
        await supabase.from('ptz_commands').delete().eq('user_id', userId);
      } catch (_) {}

      // 6. Datos del usuario en public.users
      try {
        await supabase.from('users').delete().eq('id', userId);
        debugPrint('✅ Datos de usuario eliminados');
      } catch (_) {}

      // 7. Eliminar usuario de Supabase Auth via RPC (SECURITY DEFINER)
      try {
        await supabase.rpc('delete_user');
        debugPrint('✅ Usuario Auth eliminado via RPC');
      } catch (e) {
        debugPrint('⚠️ RPC delete_user no disponible, continuando: $e');
      }
